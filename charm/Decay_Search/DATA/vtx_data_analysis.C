...
void Loop()
{
  ...
  TFile * fedrafile = TFile::Open("vertextree_thirdquarter.root");  // full
  ...
  TH1F *hsamesegMC = new TH1F("same seg ipmaxMC rmaxMC","same_segMC",10,-1,2);
   ...
   for (Long64_t ientry=0; ientry<nentries_vtx_fedra;ientry++) { // Vertex index
     ...
     if(ientry>=0/*21558 && ientry<=21570 /*|| ientry==1183*/){ // for debugging
   ...
   int first_entry=139653;//26000;
   int last_entry=139654;//nentries_vtx_fedra;
   int event = 0;
   ...
   for (Long64_t ientry=first_entry; ientry<last_entry;ientry++) {
     ...
     if(ientry%1==0)cout << "VID DS " << ientry << endl;    
       ...
       if(n>25 && probability>0.8 /*&& mean_vtx_seg>=8*/ /*&& vtx_bdt[ientry]>0*/){ // da scommentare solo se il file in input è stato testato con la bdt
	 ...
	 for(int j=0;j<height;j++){
	   for(int k=0;k<depth;k++){
		 ...
		 if(vtx_list.at(i).at(j).at(k).at(l)==vID){ // trovo il cubetto di appartenenza del vertice
		   ...
		   //cout << xpos.size() << endl;
		   if(xpos.size()>1){
		     //tie(cl_el,n_el,min_dist) = dbscan(epsilon,epsilon_phi,min_points,xpos.size(),xpos,ypos,zpos,tx,ty);
			 ...
			 if(!not_new_2vtx){
			   for(int jcl=0;jcl<n_el[icl];jcl++){
				 ...
				 log_decay << "Element " << s << " with Track ID " << trk_good_id[cl_el[icl][s]] << " and nseg " << nseg2[cl_el[icl][s]] <<  endl;
			 ...
			 if(jcl==(n_el_rel[icl]-1)){
			   ...
			   if(sum_ip>50 && sum_ka>0.01 && abs(mean_dist_vtx_rel.at(icl))>10 && !not_good){
			     ...
			     for(int s=0;s<n_el_rel[icl];s++){
			       log_decay << "Element " << s << " with Track ID " << trk_good_id[cl_el_rel[icl][s]] << " and nseg " << nseg2[cl_el_rel[icl][s]] <<  endl;
		 ...
		 if(low_seg==0){
		   ...
		   if(flag==4){
		     ...
		     for(int itrk=0;itrk<n;itrk++){
		       ...
		       mc_ty = (t__eY[itrk]-tmp_vy)/fdz;
		       ka = sqrt(pow(mc_tx - t__eTX[itrk],2) + pow(mc_ty - t__eTY[itrk],2));
		       //&& (t__eTX[itrk]>0.1 || t__eTY[itrk]>0.1)
		       if(nseg[itrk]>=8 && ip>50 && ip<1000 && fdz<10000 && fdz>500  && ka<0.2){ // stabilire se necessario
			 if(!trk_found){
			   log_decay << "DS info on secondary vertex " << endl;
			   log_decay <<"Nearby linked vertex found " << vtx_list_nearby.at(i).at(j).at(k).at(l) << " at distance " << (vz-tmp_vz) << " with " << n << " tracks " << endl;
			   trk_found=true;
			 } 
		   ...
		   if((flag==0 || flag==3) && n<4){ // vertici non linkati (magari neutral charmed daughters)
			 ...
			 if(!trk_found){
			   log_decay << "DS info on secondary vertex " << endl;
			   log_decay <<"Nearby neutral vertex found " << vtx_list_nearby.at(i).at(j).at(k).at(l) << " at distance " << (vz-tmp_vz) << " with " << n << " tracks " << endl;
			    trk_found=true;
			 }
			 log_decay << "Linked to the vertex " << vtx_list_nearby.at(i).at(j).at(k).at(l)  << " Track in "<< i << " " << j << " " << k << " " <<   l <<  " with TrkID " << TrackID[itrk] << " and " << nseg[itrk] << " segments" << " and zpos " << incoming[itrk] << endl;
			 log_decay << "track_pos (XYZ) "<< t__eX[itrk] << " " << t__eY[itrk] << " " << t__eZ[itrk]  << endl;
