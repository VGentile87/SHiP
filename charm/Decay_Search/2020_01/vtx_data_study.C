...
void Fill2ndTrack(int ife_id, float ix, float iy, float iz, float itx, float ity, int inseg, int inpl, int inholes, int imaxgap, float iip, float ika, int imc_ev, int imc_tid, int imc_pid, float itiprms, float itipmean, float itipmax,float itkarms, float itkamean, float itkamax, int iincoming, float ipms, float ipms_ang){
		 ...
		 if(!vtx_list_nearby.at(i).at(j).at(k).empty()){
		   ...
		   for(int l=0;l< vtx_list_nearby.at(i).at(j).at(k).size();l++){
			 ...
			 tie(pms,pms_ang) =  momentum_estimator(*aTrk);
		       ...
		       Fill2ndTrack(TrackID[itrk],t__eX[itrk],t__eY[itrk],t__eZ[itrk],t__eTX[itrk],t__eTY[itrk],nseg[itrk],npl[itrk],nholes[itrk],maxgap[itrk],ip,ka,MCEventID[itrk],MCTrackID[itrk],MCMotherID[itrk],trk_kink_consecutive[0],trk_kink_consecutive[1],trk_kink_consecutive[2],trk_kink_random[0],trk_kink_random[1],trk_kink_random[2],incoming[itrk],pms,pms_ang);
